...
ifneq (,$(findstring legacy,$(PADDING_TYPE)))
	python $(PBP_TOOL) -j $(KEY_PATH)/root_prvk.pem -func keyhash_legacy -o $(D_BIN)/keyhash.bin
...
ifeq ($(CFG_PRELOADER_EXT_COMMON_BUILD), 1)
	...
	python $(SECLIB2OBJ_TOOL) -b $(SECLIB_OBJ_LIST_FILE) -r $(D_ROOT) -p $(DEVINFO_OBJ_DIR) -s $(DEVINFO_LIB)
...
show_feature:
	...
	echo 'CFG_WATCHDATA_TEE_SUPPORT'= $(CFG_WATCHDATA_TEE_SUPPORT) ; \
