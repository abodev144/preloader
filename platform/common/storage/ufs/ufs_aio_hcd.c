...
int ufs_aio_rpmb_get_rw_size(void)
{
	...
	ret = hba->read_descriptor(hba, QUERY_DESC_IDN_GEOMETRY, 0, buf, QUERY_DESC_GEOMETRY_MAX_SIZE);
...
u64 ufs_aio_rpmb_get_lu_size(void)
{
	...
	ret = hba->read_descriptor(hba, QUERY_DESC_IDN_UNIT, UFS_UPIU_RPMB_WLUN, buf, QUERY_DESC_UNIT_MAX_SIZE);
