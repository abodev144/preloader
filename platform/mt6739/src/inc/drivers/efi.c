...
static void get_part_info_from_entry(u32 part_id, part_t *part_ptr, gpt_entry *entry)
{
    ...
    efi_info("%sname=%s, start_sect=0x%x, nr_sects=0x%x\n", TAG, part_ptr->info->name,
    ...
    while (entries_read_size > 0) {
        ...
        for (i = 0; i < entries_cnt && found == 0; i++) {
            if(!entries[i].starting_lba)
                break;
            get_part_info_from_entry(part_id, part_ptr, &entries[i]);
...
static int parse_gpt_header(u32 part_id, u64 header_lba, u8 *header_buf, u8 *entries_buf)
{
    ...
    for (i = 0; i < header->num_partition_entries; i++) {
        ...
        get_part_info_from_entry(part_id, &part_ptr[i], &entries[i]);
    }
...
find:
    efi_err("%sSuccess to find valid GPT.\n", TAG);
    return 0;
}
