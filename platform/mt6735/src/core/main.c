...
if (1 == aarch64_slt_done())
{
    ...
    *jump_addr = addr;
    ...
    u32 pid = EMMC_PART_UNKNOWN;
    if (mmc_get_boot_part(&pid)) {
        print("invalid boot part: %d\n", pid);
    }
    if(pid == EMMC_PART_BOOT1)
        print("Current boot: Preloader_a\n");
    else if(pid == EMMC_PART_BOOT2)
        print("Current boot: Preloader_b\n");
    else
        print("Current boot: %d\n", pid);
            ...
            if (get_bootup_status(ab_suffix)) { //boot_successfully == 1
                ...
                if (ab_retry > 0) {
                    ...
                    if (!memcmp(ab_suffix, BOOTCTRL_SUFFIX_A, 2)) {
                        ...
                        if (mmc_set_boot_part(EMMC_PART_BOOT2)) {
                        ...
                        if (mmc_set_boot_part(EMMC_PART_BOOT1)) {
